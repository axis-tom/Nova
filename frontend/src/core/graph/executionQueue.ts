...
class QueueItem {
  ...
  constructor(id: string, task: () => Promise<unknown>, priority: number = 0, metadata: Record<string, unknown> = {}) {
    this.id = id
    this.task = task
 ...
 * @param options - 选项
 */
export async function enqueue(task: () => Promise<unknown>, options: { id?: string; priority?: number; metadata?: Record<string, unknown> } = {}): Promise<unknown> {
  ...
  // 返回 Promise，等待任务完成，增加了轮询机制的健壮性
  return new Promise((resolve, reject) => {
    let attempts = 0;
    const maxAttempts = 100; // 最多等待 10 秒 (100ms * 100)
    const timer = setInterval(() => {
      ...
      if (item && item instanceof QueueItem) {
        // 3. 任务项状态为已完成
      ...
      // 5. 如果任务项不在队列中，也没有完成/失败记录，可能是竞态条件，等待一下再检查
      if (!item && !queueState.completed.has(taskId) && !queueState.failed.has(taskId)) {
        if (attempts >= maxAttempts) {
          clearInterval(timer);
          reject(new Error('任务状态未知：任务可能已被清理或超时'));
        }
        return;
      }

      // 6. 最终兜底，如果超过最大尝试次数，中止等待
      if (attempts >= maxAttempts) {
        clearInterval(timer);
        reject(new Error('任务状态未知：等待超时'));
      }
    }, 100);
  });
}
 ...
 * @param options - 选项
 */
export async function batchEnqueue(tasks: Array<() => Promise<unknown>>, options: { ids?: string[]; priorities?: number[]; metadata?: Record<string, unknown>[] } = {}): Promise<Array<{ success: boolean; result?: unknown; error?: unknown }>> {
  console.log(`批量添加 ${tasks.length} 个任务到队列`)
  
  const promises = tasks.map((task, index) => {
    const taskOptions: { id?: string; priority?: number; metadata?: Record<string, unknown> } = {
 ...
 * @param queueItem - 队列项
 */
async function executeTask(queueItem: QueueItem): Promise<void> {
 ...
 * 获取下一个队列项
 */
function getNextQueueItem(): QueueItem | null {
  if (queueState.items.length === 0) {
    return null
  }
 ...
 * @param queueItem - 队列项
 */
function insertByPriority(queueItem: QueueItem): void {
  let inserted = false
 ...
 * @param taskId - 任务ID
 */
function isInQueue(taskId: string): boolean {
 ...
 * @param taskId - 任务ID
 */
function getQueueItem(taskId: string): QueueItem | null {
  // 在队列中查找
 ...
 * @param queueItem - 队列项
 */
function updateStats(queueItem: QueueItem): void {
 ...
 * 获取队列状态
 */
export function getQueueStatus(): { pending: number; running: number; completed: number; failed: number; isProcessing: boolean; stats: QueueStats } {
  return {
 ...
 * @param taskId - 任务ID
 */
export function getQueueItemDetails(taskId: string): Record<string, unknown> | null {
  const item = getQueueItem(taskId)
  if (!item) {
    return null
  }
 ...
 * 获取所有队列项
 */
export function getAllQueueItems(): Array<Record<string, unknown>> {
 ...
 * @param newConfig - 新配置
 */
export function updateQueueConfig(newConfig: Partial<typeof QUEUE_CONFIG>): void {
 ...
 * 获取队列配置
 */
export function getQueueConfig(): typeof QUEUE_CONFIG {
  return { ...QUEUE_CONFIG }
}