 ...
 * @returns {Promise} 执行结果
 */
async function executeByType(node, input, config) {
  switch (node.type) {
 ...
 * 执行调研节点
 */
async function executeResearch(node, input) {
 ...
 * 执行分析节点
 */
async function executeAnalysis(node, input) {
 ...
 * 执行生成节点
 */
async function executeGeneration(node, input) {
 ...
 * 执行优化节点
 */
async function executeOptimization(node, input) {
 ...
 * 执行检查节点
 */
async function executeCheck(node, input) {
 ...
 * 执行评估节点
 */
async function executeAssessment(node, input) {
 ...
 * 生成Listing（电商场景）
 */
async function generateListing(node, input) {
 ...
 * 生成图片
 */
async function generateImages(node, input) {
 ...
 * 分析评论
 */
async function analyzeReviews(node, input) {
 ...
 * 执行通用节点
 */
async function executeGeneric(node, input) {
  return {
    type: 'generic',
    nodeId: node.id,
    nodeType: node.type,
 ...
 * @param {Object} inputs - 输入数据映射
 * @returns {Promise} 执行结果映射
 */
export async function batchExecute(nodes, inputs = {}) {
 ...
 */
export function validateInput(node, input) {
...
function validateListingInput(input) {
  ...
  return input.product && input.category
}
...
function validateImageInput(input) {
  if (!input) return false
  return input.prompt || input.reference
}
...
function validateReviewInput(input) {
  if (!input) return false
  return input.reviews && Array.isArray(input.reviews)
}
 ...
 * @returns {Object} 执行配置
 */
export function getExecutionConfig(nodeType) {
...
export function getSupportedNodeTypes() {
  return Object.keys(NODE_TYPES).map(key => NODE_TYPES[key])
}
