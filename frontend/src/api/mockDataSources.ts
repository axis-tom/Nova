...
interface MockDataSourceTypeConfig {
  name: string;
...
// Mock数据源类型定义
const mockDataSourceTypes: Record<string, MockDataSourceTypeConfig> = {
  email: {
    name: '邮箱',
    fields: [
  ...
  rss: {
    ...
    fields: [
      ...
      { name: 'categories', label: '分类标签', type: 'text', required: false, placeholder: '用逗号分隔，如：科技,金融' }
    ]
  },
  weibo: {
    name: '微博',
    fields: [
      ...
      { name: 'accounts', label: '监控账号', type: 'text', required: false, placeholder: '用逗号分隔，如：technews,ai_research' },
  ...
  financial: {
    ...
    fields: [
      ...
      { name: 'endpoints', label: '数据端点', type: 'text', required: false, placeholder: '用逗号分隔，如：stock,forex,crypto' },
 ...
 * 获取所有数据源（mock）
 */
export async function getDataSources(): Promise<MockDataSource[]> {
 ...
 * 获取单个数据源详情（mock）
 */
export async function getDataSource(id: string | number): Promise<MockDataSource> {
  console.log(`[Mock API] 获取数据源详情 ID: ${id}`);
  await delay(200);
  const source = mockDataSources.find(s => s.id === parseInt(String(id)));
  if (!source) {
 ...
 * 创建数据源（mock）
 */
export async function createDataSource(data: Record<string, unknown>): Promise<MockDataSource & { message: string }> {
 ...
 * 更新数据源（mock）
 */
export async function updateDataSource(id: string | number, data: Record<string, unknown>): Promise<MockDataSource & { message: string }> {
  console.log(`[Mock API] 更新数据源 ID: ${id}`, data);
  await delay(300);
  
  const sourceIndex = mockDataSources.findIndex(s => s.id === parseInt(String(id)));
  if (sourceIndex === -1) {
    throw new Error(`数据源 ${id} 不存在`);
  }
  
  const updatedSource: MockDataSource = {
    ...mockDataSources[sourceIndex],
    ...data as unknown as Partial<MockDataSource>,
 ...
 * 删除数据源（mock）
 */
export async function deleteDataSource(id: string | number): Promise<{ success: boolean; message: string }> {
 ...
 * 测试数据源连接（mock）
 */
export async function testDataSource(config: Record<string, unknown>): Promise<{ success: boolean; message: string }> {
 ...
 * 获取数据源类型（mock）
 */
export async function getDataSourceTypes(): Promise<Record<string, MockDataSourceTypeConfig>> {
 ...
 * 使用环境参数的mock API包装器
 */
export function createMockDataSourceApi(envStore: { currentEnv: string }): {
  getDataSources: (params?: Record<string, unknown>) => Promise<MockDataSource[]>;
  getDataSource: (id: string | number, params?: Record<string, unknown>) => Promise<MockDataSource>;
  createDataSource: (data: Record<string, unknown>, params?: Record<string, unknown>) => Promise<MockDataSource & { message: string }>;
  updateDataSource: (id: string | number, data: Record<string, unknown>, params?: Record<string, unknown>) => Promise<MockDataSource & { message: string }>;
  deleteDataSource: (id: string | number, params?: Record<string, unknown>) => Promise<{ success: boolean; message: string }>;
  testDataSource: (config: Record<string, unknown>, params?: Record<string, unknown>) => Promise<{ success: boolean; message: string }>;
  getDataSourceTypes: (params?: Record<string, unknown>) => Promise<Record<string, MockDataSourceTypeConfig>>;
} {
  return {
