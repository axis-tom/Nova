...
import { getTraceDebugInfo, replayTrace, retryTrace } from '@/api/graph'
...
interface DebugInfo {
  ...
  execution_timeline: TimelineNode[]
  suggestions: string[]
}
...
const showTimelineChart = ref<boolean>(true)
...
const loadDebugInfo = async () => {
  try {
    ...
    debugInfo.value = (response as Record<string, unknown>).data as DebugInfo
    
  } catch (err: unknown) {
    const e = err as { response?: { data?: { detail?: string } }; message?: string }
    error.value = e.response?.data?.detail || e.message || '加载调试信息失败'
...
const getErrorSeverity = (errorSummary: ErrorSummary): string => {
...
const getErrorSummaryText = (errorSummary: ErrorSummary): string => {
...
const handleRetry = async () => {
  ...
  try {
    ...
    const response2 = await retryTrace(props.traceId!) as Record<string, unknown>
    
    ElMessage.success('重试请求已发送')
    console.log('重试结果:', response2.data)
...
const exportDebugInfo = () => {
  ...
  a.href = url
  a.download = `debug-report-${props.traceId ?? 'unknown'}.json`
