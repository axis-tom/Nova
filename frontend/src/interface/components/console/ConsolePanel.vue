  ...
  const doResize = (moveEvent: MouseEvent) => {
    ...
    if (props.width !== 'auto') {
      const newWidth = Math.max(
        parseInt(String(props.minWidth)) || 200,
        (typeof startWidth === 'number' ? startWidth : parseInt(String(startWidth))) + deltaX
      )
    ...
    if (props.height !== 'auto') {
      const newHeight = Math.max(
        parseInt(String(props.minHeight)) || 100,
        (typeof startHeight === 'number' ? startHeight : parseInt(String(startHeight))) + deltaY
      )
