        ...
        <el-form-item label="场景选择" prop="scenario">
          ...
          >
            <el-option
              ...
              :key="(scenario as ScenarioItem).scenario"
              :label="(scenario as ScenarioItem).scenario"
              :value="(scenario as ScenarioItem).scenario"
            >
              <div class="scenario-option">
                <span class="scenario-name">{{ (scenario as ScenarioItem).scenario }}</span>
                <span class="scenario-desc">{{ (scenario as ScenarioItem).description }}</span>
              </div>
            </el-option>
          </el-select>
          <div class="form-hint">
      ...
      <div v-if="executionResult" class="execution-result">
        ...
        <div class="result-summary">
          <div class="summary-item">
            <label>执行状态:</label>
            <span class="status-badge" :class="(executionResult as ExecutionResult).success ? 'success' : 'error'">
              {{ (executionResult as ExecutionResult).success ? '成功' : '失败' }}
            </span>
          </div>
          <div v-if="(executionResult as ExecutionResult).trace_id" class="summary-item">
            <label>Trace ID:</label>
            <span class="trace-id">{{ (executionResult as ExecutionResult).trace_id }}</span>
            <el-button
              type="text"
              size="small"
              icon="el-icon-link"
          ...
          <div v-if="(executionResult as ExecutionResult).execution_path" class="summary-item">
            <label>执行路径:</label>
            <span>{{ ((executionResult as ExecutionResult).execution_path as string[]).join(' → ') }}</span>
          </div>
        </div>
        ...
        <div class="result-details">
          <el-tabs v-model="activeResultTab">
            <el-tab-pane label="结果数据" name="result">
              <pre class="result-json">{{ JSON.stringify((executionResult as ExecutionResult).result, null, 2) }}</pre>
            </el-tab-pane>
            <el-tab-pane label="原始响应" name="raw">
              <pre class="result-json">{{ JSON.stringify(executionResult, null, 2) }}</pre>
            </el-tab-pane>
            <el-tab-pane v-if="(executionResult as ExecutionResult).error" label="错误信息" name="error">
              <div class="error-message">
                <i class="el-icon-warning"></i>
                <span>{{ (executionResult as ExecutionResult).error }}</span>
              </div>
            </el-tab-pane>
          </el-tabs>
        </div>
      ...
      <div v-if="executionHistory.length > 0" class="execution-history">
        ...
        <div class="history-list">
          <div
            ...
            :key="(item as HistoryItem).timestamp"
            class="history-item"
            @click="loadHistoryItem(item as HistoryItem)"
          >
            <div class="history-main">
              <span class="history-scenario">{{ (item as HistoryItem).scenario }}</span>
              <span class="history-time">{{ formatTime((item as HistoryItem).timestamp) }}</span>
            </div>
            <div class="history-details">
              <span class="history-status" :class="(item as HistoryItem).success ? 'success' : 'error'">
                {{ (item as HistoryItem).success ? '成功' : '失败' }}
              </span>
              <span v-if="(item as HistoryItem).trace_id" class="history-trace">
                Trace: {{ ((item as HistoryItem).trace_id as string).substring(0, 8) }}...
              </span>
            </div>
          </div>
        </div>
...
import { runGraph, listScenarios } from '@/api/graph'
...
// 响应式数据
const runFormRef = ref<HTMLElement | null>(null)
...
const activeResultTab = ref<string>('result')
const jsonError = ref<string>('')

// 表单验证规则
const runFormRules: Record<string, unknown[]> = {
  scenario: [
...
const loadScenarios = async () => {
  try {
    const response = await listScenarios() as Record<string, unknown>
    const details = response.details as Record<string, Record<string, unknown>>
    scenarios.value = Object.entries(details).map(([name, details]) => ({
      scenario: name,
      description: (details.description as string) || '',
      node_count: ((details.nodes as unknown[])?.length) || 0
    }))
  } catch (error: unknown) {
...
const onScenarioChange = (scenario: string) => {
  // 可以根据场景自动填充一些默认参数
...
const executeGraph = async () => {
  ...
  try {
    // 验证表单
    await (runFormRef.value as unknown as { validate: () => Promise<void> }).validate()
    ...
    const response = await runGraph(requestData) as ExecutionResult
    ...
    ElMessage.error('Graph执行失败: ' + (e.message || '未知错误'))
...
const loadHistoryItem = (item: HistoryItem) => {
  ...
  runForm.value.input = item.result as Record<string, unknown>
  runForm.value.inputJson = JSON.stringify(item.result, null, 2)
  executionResult.value = item as unknown as ExecutionResult
}
