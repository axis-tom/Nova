      ...
      <div class="controls-left">
        ...
        <div class="trace-info" v-if="currentTrace">
          <span class="info-item">
            ...
            {{ currentTrace.session.graph_name || currentTrace.session.graph_id }}
          </span>
          <span class="info-item">
            <i class="el-icon-time"></i>
            {{ formatDuration(currentTrace.session.duration) }}
          </span>
          <span class="info-item">
            <i class="el-icon-s-check"></i>
            {{ currentTrace.nodes.filter(n => n.status === 'completed').length }}/{{ currentTrace.nodes.length }}
          </span>
          <span class="info-item">
            <i class="el-icon-user"></i>
            {{ currentTrace.session.status }}
          </span>
        </div>
      </div>
    ...
    <div class="integration-main">
          ...
          <div class="trace-summary">
            <div class="summary-grid">
              <div class="summary-item">
                ...
                <span class="summary-value">{{ currentTrace.session.graph_name || currentTrace.session.graph_id }}</span>
              </div>
              <div class="summary-item">
                <span class="summary-label">状态</span>
                <span class="summary-value" :class="currentTrace.session.status">
                  {{ getStatusText(currentTrace.session.status) }}
                </span>
              </div>
              <div class="summary-item">
                <span class="summary-label">开始时间</span>
                <span class="summary-value">{{ formatTime(currentTrace.session.start_time) }}</span>
              </div>
              <div class="summary-item">
                <span class="summary-label">持续时间</span>
                <span class="summary-value">{{ formatDuration(currentTrace.session.duration) }}</span>
              </div>
              <div class="summary-item">
                <span class="summary-label">节点总数</span>
                <span class="summary-value">{{ currentTrace.summary?.node_statistics?.total || currentTrace.nodes.length }}</span>
              </div>
              <div class="summary-item">
                <span class="summary-label">成功率</span>
                <span class="summary-value success">
                  {{ ((currentTrace.summary?.node_statistics?.success_rate || 0) * 100).toFixed(1) }}%
                </span>
              </div>
            </div>
          </div>
      ...
      <div class="graph-panel">
        ...
        <div v-if="selectedNodeDetail" class="node-detail-panel">
          <div class="panel-header">
            <h4>节点详情: {{ selectedNodeDetail.node_name || selectedNodeDetail.node_id }}</h4>
            <el-button
              type="text"
              icon="el-icon-close"
            ...
            <div class="detail-section">
              ...
              <div class="detail-grid">
                <div class="detail-item">
                  ...
                  <span class="detail-value">{{ selectedNodeDetail.node_id }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">执行顺序</span>
                  <span class="detail-value">{{ selectedNodeDetail.execution_order }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">智能体</span>
                  <span class="detail-value">{{ selectedNodeDetail.agent_name }} ({{ selectedNodeDetail.agent_type || '未知类型' }})</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">状态</span>
                  <span class="detail-value" :class="selectedNodeDetail.status">
                    {{ getNodeStatusText(selectedNodeDetail.status) }}
                  </span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">开始时间</span>
                  <span class="detail-value">{{ formatTime(selectedNodeDetail.start_time) }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">结束时间</span>
                  <span class="detail-value">{{ formatTime(selectedNodeDetail.end_time) }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">持续时间</span>
                  <span class="detail-value">{{ formatDuration(selectedNodeDetail.duration) }}</span>
                </div>
              </div>
            </div>

            <!-- 输入数据 -->
            <div class="detail-section" v-if="selectedNodeDetail.input_data">
              <h5>输入数据</h5>
              <div class="data-viewer">
                <pre>{{ formatJson(selectedNodeDetail.input_data) }}</pre>
              </div>
            </div>

            <!-- 输出数据 -->
            <div class="detail-section" v-if="selectedNodeDetail.output_data">
              <h5>输出数据</h5>
              <div class="data-viewer">
                <pre>{{ formatJson(selectedNodeDetail.output_data) }}</pre>
              </div>
            </div>

            <!-- 错误信息 -->
            <div class="detail-section" v-if="selectedNodeDetail.error_message">
              <h5>错误信息</h5>
              <div class="error-viewer">
                <div class="error-message">{{ selectedNodeDetail.error_message }}</div>
                <div v-if="selectedNodeDetail.error_details" class="error-details">
                  <pre>{{ formatJson(selectedNodeDetail.error_details) }}</pre>
                </div>
              </div>
            </div>
          </div>
...
import { getTraceDetails, replayTrace } from '@/api/graph.js'
...
const loadTraceData = async () => {
  ...
  try {
    ...
    const response = await getTraceDetails(props.traceId)
    ...
    if (savedData) {
      ...
      if (hoursDiff < 1) { // 1小时内有效
        ...
        if (selectedNodeId.value && currentTrace.value) {
          const node = currentTrace.value.nodes.find(n => n.id === selectedNodeId.value)
          if (node) {
            selectedNodeDetail.value = node
          }
        }
