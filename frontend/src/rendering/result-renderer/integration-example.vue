...
const showResultContainer = ref<boolean>(true)
const showRendererDemo = ref<boolean>(false)
const showEditorDemo = ref<boolean>(false)
const activeEditor = ref<string>('text')
const editorContent = ref<string>('')
const activeCodeTab = ref<string>('container')
const currentResultType = ref<string>('')
const eventBusStatus = ref<string>('已连接')
const dataFlowStatus = ref<string>('正常')
const operationLogs = ref<unknown[]>([])
...
<script setup>
import ResultContainer from '@/rendering/result-renderer/ResultContainer.vue'
    ...
    table: {
      ...
      content: {
        ...
        data: [
          ...
          { month: '四月', revenue: '¥192,000', growth: '+14%', target: '¥160,000' }
        ]
      },
      editable: true,
      meta: {
    ...
    chart: {
      ...
      content: {
        ...
        datasets: [
          {
            label: '收入',
            data: [120000, 145000, 168000, 192000]
          },
          {
            label: '目标',
            data: [110000, 120000, 140000, 160000]
          }
        ]
      },
      editable: false,
      meta: {
        sourceNode: 'data_analyzer',
        timestamp: Date.now()
      }
    },
    image: {
      ...
      content: {
        title: '设计对比',
        images: [
          { url: 'https://via.placeholder.com/400x300?text=Version+A', label: '版本A' },
          { url: 'https://via.placeholder.com/400x300?text=Version+B', label: '版本B' }
        ]
      },
      editable: false,
      meta: {
        sourceNode: 'design_compare',
        timestamp: Date.now()
      }
    }
  }
  
  const result = testResults[type] || testResults.text
  currentResultType.value = type
  addLog('info', '🚀', `模拟Graph输出: ${type}`)
  addLog('info', '📦', '结果已发送到ResultContainer')
...
const toggleEditable = () => {
  ...
  addLog('info', isEditable.value ? '🔓' : '🔒', `编辑状态: ${isEditable.value ? '启用' : '禁用'}`)
}

const clearResults = () => {
  currentResultType.value = ''
  addLog('info', '🗑️', '清除所有结果')
  eventBus.emit(eventTypes.RESULT_UPDATE, {
    action: 'clear',
    timestamp: Date.now()
  })
...
const previewRenderer = (renderer) => {
  addLog('info', '🎨', `预览渲染器: ${renderer.name}`)
  simulateGraphOutput(renderer.type)
}

const getEditorPlaceholder = (type) => {
  const placeholders = {
    text: '请输入文本内容...',
    markdown: '# 请输入Markdown内容...',
    json: '{ "key": "value" }'
  }
  return placeholders[type] || '请输入内容...'
}

const handleEditorEdit = (data) => {
  addLog('info', '✏️', '编辑器编辑中...')
}

const handleEditorSave = (data) => {
  addLog('info', '💾', `编辑器保存: ${data.substring(0, 50)}...`)
...
const handleEditorCancel = () => {
  addLog('info', '⏹️', '编辑器取消编辑')
}

const addLog = (type, icon, message) => {
  const now = new Date()
  const time = now.toLocaleTimeString('zh-CN', { hour12: false })
  operationLogs.value.unshift({ type, icon, message, time })
...
// 生命周期
onMounted(() => {
  addLog('success', '✅', 'Result系统集成示例已加载')
  addLog('info', '📡', '事件总线已连接')
  addLog('info', '🎯', '等待Graph输出...')
})
</script>
</parameter>
