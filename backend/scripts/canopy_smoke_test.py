"""
Canopy API Smoke Test
每次只测一个端点，默认测 /product（1 credit）。

用法:
    cd backend
    python -m scripts.canopy_smoke_test              # 测 /product (1 credit)
    python -m scripts.canopy_smoke_test --search      # 测 /search  (1 credit)
    python -m scripts.canopy_smoke_test --reviews     # 测 /reviews (1 credit)
    python -m scripts.canopy_smoke_test --all         # 全部三个  (3 credits)
"""
...
def test_product(connector):
    ...
    for name, check in [
        ...
        ("Top reviews", isinstance(product.get("top_reviews"), list)),
        ("Specifications", len(product.get("specifications", {})) > 0),
        ("Category", bool(product.get("category_name"))),
        ("Brand", bool(product.get("brand"))),
        ("is_prime", isinstance(product.get("is_prime"), bool)),
    ]:
        total += 1
        flag = "✅" if check else "❌"
        if check: ok += 1
...
async def main():
    ...
    connector = CanopyConnector(api_key)
    all_ok = True
