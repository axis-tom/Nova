...
from backend.agents.strategist import *
from backend.agents.conversation import *
from backend.agents.quality import *
from backend.agents.learning import *
...
class Orchestrator:
    """调度引擎：加载SOP，执行智能体链"""
    def __init__(self):
        self.sop_dir = Path(__file__).parent.parent / "sop"
            ...
            if step_type == "sequential":
                ...
                for agent_name in agent_names:
                    result = await self._execute_agent_with_retry(
                        agent_name, step_input, user_id, trace_id, timeout, retry
                    )
                    # 将结果合并到上下文
        ...
        for attempt in range(retry_limit + 1):
            try:
                result = await asyncio.wait_for(
                    self._execute_agent(agent_name, input_data, user_id, trace_id),
                    timeout=timeout
                )
                return result
